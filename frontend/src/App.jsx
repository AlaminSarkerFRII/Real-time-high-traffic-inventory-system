...
function App() {
  ...
  const handlePurchase = async () => {
    ...
    try {
      const res = await axios.post(`/api/purchase/${reservation.id}`, { userId })
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-4">
      ...
      <h1 className="text-3xl font-bold text-center mb-8">Real-time Inventory System</h1>
      ...
      <div className="max-w-4xl mx-auto">
          ...
          {drops.map((drop) => (
            <div key={drop.id} className="bg-white rounded-lg shadow-md p-4">
              ...
              {drop.recentPurchases && drop.recentPurchases.length > 0 && (
                <div className="mb-4">
                  <h3 className="text-sm font-medium text-gray-700 mb-2">Recent Purchases:</h3>
              ...
              {drop.availableStock > 0 && (
                ...
                >
                  {loadingStates[`reserve-${drop.id}`] ? 'Reserving...' : 'Reserve'}
                </button>
              )}
            </div>
          ))}
        ...
        {reservation && (
          <div className="mt-8 bg-white rounded-lg shadow-md p-4">
            ...
            <p className="text-red-600 font-bold">Time remaining: {countdown}</p>
            <button
            ...
            >
              {loadingStates.purchase ? 'Purchasing...' : reservationExpired ? 'Reservation Expired' : 'Purchase Now'}
            </button>
          </div>
        )}
